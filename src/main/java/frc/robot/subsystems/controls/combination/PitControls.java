...
public class PitControls {
    ...
    private static void setupElevatorPitControls(Elevator elevator) {
        ...
        Command runVoltsCommand = new MotorPitCommand((Motor) elevatorSubsystem, "Pit/" + elevatorSubsystem.getName() + "/Commands/Run Voltage/Volts Amount");
        SmartDashboard.putData("Pit/" + elevatorSubsystem.getName() + "/Commands/Run Voltage/Command", runVoltsCommand);
    }
    private static void setupArmPitControls(Arm arm){

        SubsystemBase armSubsystem = (SubsystemBase) arm;
        
        Command runVoltsCommand = new MotorPitCommand((Motor) armSubsystem, "Pit/" + armSubsystem.getName() + "/Commands/Run Voltage/Volts Amount");
        SmartDashboard.putData("Pit/" + armSubsystem.getName() + "/Commands/Run Voltage/Command", runVoltsCommand);
    }
}